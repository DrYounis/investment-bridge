...
<div style="padding:32px 24px">
  ...
  <div style="border:2px solid #c9a84c;border-radius:16px;padding:24px;background:#faf8f2">
    <h2 style="color:#0a0f1e;font-size:18px;margin:0 0 16px 0;text-align:center">📅 دعوة لحضور اللقاء ${meeting.meetingNumber} — ${meeting.dateStr}</h2>
...
export async function GET() {
  ...
  const { error } = await resend.emails.send({
    ...
    subject: `🚀 مرحباً بك في مرفأ + 🔔 لقاء ${meeting.meetingNumber} — ${meeting.dateStr} | ${meeting.case}`,
    html: buildEmail(),
  });

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ success: true, meeting: { number: meeting.meetingNumber, case: meeting.case, topic: meeting.topic, date: meeting.dateStr } });
}
