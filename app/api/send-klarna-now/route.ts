...
export async function GET(request: NextRequest) {
  ...
  try {
    const pdfPath = join(process.cwd(), 'public', PDF_FILENAME);
    ...
    await resend.emails.send({
      ...
      html: `<div style="font-family: sans-serif; padding: 20px;"><h2>تقرير إرسال مادة Klarna AI-First</h2><p>أُرسل ${sent} من ${subscribers.length}.</p><pre>${JSON.stringify(failedRecipients, null, 2)}</pre></div>`,
    }).catch(() => {});

    return NextResponse.json({ success: true, sent, failed, total: subscribers.length, failedRecipients });
  } catch (err: unknown) {
