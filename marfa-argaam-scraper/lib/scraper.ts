...
function formatDate(text: string | null | undefined): string {
  ...
  try {
    // Try parsing Arabic/Hijri dates — fall back to today
...
/**
 * Scrape article listing cards from Argaam news homepage.
 * Fallback selectors accommodate different page layouts Argaam may use.
 */
...
/**
 * Navigate to an article page and extract full content.
 */
...
/**
 * Scrape Argaam financial news.
 *
 * @param maxArticles - Maximum number of articles to scrape (default 5)
 * @returns Array of article objects with full content
 */
export async function scrapeArgaamNews(
  ...
  try {
    browser = await puppeteer.launch({
      headless: DEFAULT_OPTIONS.headless,
      args: [
        '--no-sandbox',
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      if (card.url) {
        ...
        await delay(DEFAULT_OPTIONS.delayMs);
      }
